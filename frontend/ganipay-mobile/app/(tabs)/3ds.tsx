...
export default function ThreeDSScreen() {
  ...
  const [secondsLeft, setSecondsLeft] = useState(180); // ✅ 3 dk
  ...
  }, [loading, payload, verifying, secondsLeft]);
  ...
  if (!payload || !summary) {
    return (
      <View style={[styles.page, styles.center, { padding: 18 }]}>
        ...
        <Text style={[styles.mutedText, { textAlign: "center" }]}>Go back and start the top up again.</Text>
  ...
  return (
    <View style={styles.page}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.bankSub}>OTP is simulated. Any 6 digits will be accepted.</Text>
        ...
        >
          {verifying ? (
            <>
              <ActivityIndicator />
              <Text style={styles.primaryText}>Confirming...</Text>
            </>
          ) : (
            <>
