    ...
    public async Task Handle(IJobClient client, IJob job)
    {
        try
        {
            ...
            object completeVars = job.Type switch
            {
                ...
                // 4) Notify (mock) - iki job type aynı handler
                ...
                {
                    // senin BPMN’de complete sonrası success event’e gidiyor (Flow_0kenwne)
                    persistOk = true,
                ...
                // default: bilmediğimiz job type gelirse fail yerine complete edip trace bas (dev ortamı)
                _ => new
                {
